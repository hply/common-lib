...
public class KeyboardPatch {
    ...
    private KeyboardPatch(Activity activity, Dialog dialog, String tag, View contentView) {
        ...
        this.mBarParams = dialog != null ? StatusBar.with(activity, dialog, tag).getBarParams()
                : StatusBar.with(activity).getBarParams();
        if (mBarParams == null) {
            throw new IllegalArgumentException("先使用StatusBar init初始化");
        }
    }
