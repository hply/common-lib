package com.gogoal.adapter.recyclerView.callback;
...
import com.gogoal.adapter.recyclerView.BaseItemDraggableAdapter;
...
//    private static final String TAG = ItemDragAndSwipeCallback.class.getSimpleName();
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        if (actionState == ItemTouchHelper.ACTION_STATE_DRAG
            ...
            viewHolder.itemView.setTag(R.id.BaseCommonAdapter_dragging_support, true);
        } else if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE
                && !isViewCreateByAdapter(viewHolder)) {
            mAdapter.onItemSwipeStart(viewHolder);
            viewHolder.itemView.setTag(R.id.BaseCommonAdapter_swiping_support, true);
        }
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getTag(R.id.BaseCommonAdapter_dragging_support) != null
                && (Boolean) viewHolder.itemView.getTag(R.id.BaseCommonAdapter_dragging_support)) {
            mAdapter.onItemDragEnd(viewHolder);
            viewHolder.itemView.setTag(R.id.BaseCommonAdapter_dragging_support, false);
        }
        if (viewHolder.itemView.getTag(R.id.BaseCommonAdapter_swiping_support) != null
                && (Boolean) viewHolder.itemView.getTag(R.id.BaseCommonAdapter_swiping_support)) {
            mAdapter.onItemSwipeClear(viewHolder);
            viewHolder.itemView.setTag(R.id.BaseCommonAdapter_swiping_support, false);
        }
    }
    ...
    private boolean isViewCreateByAdapter(RecyclerView.ViewHolder viewHolder) {
        ...
        return type == BaseCommonAdapter.HEADER_VIEW || type == BaseCommonAdapter.LOADING_VIEW
                || type == BaseCommonAdapter.FOOTER_VIEW || type == BaseCommonAdapter.EMPTY_VIEW;
    }
}
