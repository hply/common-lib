...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    public BaseViewHolder setImageUrl(Object context, @IdRes int viewId, @NonNull String imageUrl) {
        if (context == null || TextUtils.isEmpty(imageUrl)) {
            return this;
        }

        RequestOptions options = RequestOptions.placeholderOf(R.mipmap.default_image).dontAnimate()
                .dontTransform()
                .placeholder(R.mipmap.default_image)
                .diskCacheStrategy(DiskCacheStrategy.ALL);
        ImageView imageView = getView(viewId);
        if (context instanceof Activity) {
            Glide.with((Activity) context).load(imageUrl).apply(options).into(imageView);
        } else if (context instanceof Fragment) {
            Glide.with((Fragment) context).load(imageUrl).apply(options).into(imageView);
        } else if (context instanceof Context) {
            Glide.with((Context) context).load(imageUrl).apply(options).into(imageView);
        } else {
            throw new RuntimeException("第一个参数必须是Activity或者Fragment的子类");
        }
        return this;
    }
     ...
     * @return The BaseViewHolder for chaining.
     */
    protected BaseViewHolder setAdapter(BaseCommonAdapter adapter) {
