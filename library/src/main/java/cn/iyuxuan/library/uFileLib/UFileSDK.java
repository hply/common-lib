...
import cn.iyuxuan.library.uFileLib.task.GetFileAsyncTask;
import cn.iyuxuan.library.uFileLib.task.HttpAsyncTask;
import cn.iyuxuan.library.uFileLib.task.PutFileAsyncTask;
import cn.iyuxuan.library.uFileLib.task.UploadPartAsyncTask;
import cn.iyuxuan.library.uFileLib.task.WriteAsyncTask;
...
//        this.defaultUrl = "http://" + bucket + ".ufile.ucloud.cn";//使用默认域名后缀
     ...
     */
    public HttpAsyncTask putFile(final UFileRequest uFileRequest, final File file, String keyName, final UFileCallBack callback) {
        String url = this.defaultUrl + "/" + UFileUtils.urlEncode(keyName);
        Log.e("TAG","==="+url);
        myUrl=url;
...
//                        KLog.e("TAG",file.toString());
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask uploadHit(UFileRequest uFileRequest, final File file, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask getFile(UFileRequest uFileRequest, String file_name, final File saveFile, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask getFile(String downloadUrl, final File saveFile, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask headFile(UFileRequest uFileRequest, String key_name, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask deleteFile(UFileRequest uFileRequest, String key_name, final UFileCallBack callback) {
        HttpAsyncTask httpAsyncTask = getDefaultHttpAsyncTask(this.defaultUrl + "/" + UFileUtils.urlEncode(key_name), uFileRequest, callback);
        httpAsyncTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        return httpAsyncTask;
    }
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask initiateMultipartUpload(UFileRequest uFileRequest, String key_name, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask uploadPart(UFileRequest uFileRequest, String key_name, String uploadId, final File file, final int partNumber, final long blk_size, final UFileCallBack callback) {
    ...
    public class UploadPartManager{
        ...
        public void start(final UFileRequest uFileRequest,
                          ...
                          final long retryTime,
                          final Handler handler){
            httpAsyncTask = uFileSDK.uploadPart(uFileRequest, key_name, uploadId, file, partNumber, blk_size, new UFileCallBack() {
                @Override
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask finishMultipartUpload(final UFileRequest uFileRequest, String key_name, final String uploadId, final String etags, String new_key_name, final UFileCallBack callback) {
     ...
     * @return http async task 用于取消掉当前任务
     */
    public HttpAsyncTask abortMultipartUpload(UFileRequest uFileRequest, String key_name, String uploadId, UFileCallBack callback) {
    ...
    private HttpAsyncTask getDefaultHttpAsyncTask(String url, UFileRequest uFileRequest, final UFileCallBack callback) {
        Log.i(TAG, "url " + url);
     ...
     * @param callback callback
     */
    private void cb(JSONObject response, UFileCallBack callback) {
        try {
